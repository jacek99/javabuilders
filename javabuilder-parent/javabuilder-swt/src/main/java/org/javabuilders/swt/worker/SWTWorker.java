 ...
 * 
 * <p>
 * When writing a multi-threaded application using SWT, there are
 ...
 *   </li>
 *   <li> SWT components should be accessed  on the <i>Event
 ...
 *
 * <p>
 * {@code SWTWorker} is designed for situations where you need to have a long 
 * running task run in a background thread and provide updates to the UI 
 * either when done, or while processing. 
 * Subclasses of {@code SWTWorker} must implement 
 ...
 * <li>
 * <p>
 * <i>Event Dispatch Thread</i>:  All SWT related activities occur
 ...
 * processing is done in the background and when done you update a SWT 
 * component.
 *
 * <p>
...
public abstract class SWTWorker<T, V> implements Future<T>, Runnable {
    ...
    public SWTWorker(Display display) {
    	...
    	doSubmit = new DoSubmitAccumulativeRunnable(this.display);
        Callable<T> callable = 
                new Callable<T>() {
       ...
       propertyChangeSupport = new SWTPropertyChangeSupport(this,this.display);
       doProcess = null;
    ...
    protected final void publish(V... chunks) {
        synchronized (this) {
            if (doProcess == null) {
                doProcess = new AccumulativeRunnable<V>(this.display) {
                    @Override
        ...
        synchronized (this) {
            if (doNotifyProgressChange == null) {
                doNotifyProgressChange = 
                    new AccumulativeRunnable<Integer>(this.display) {
                        @Override
    ...
    private static class DoSubmitAccumulativeRunnable 
         ...
         * @param display SWT display 
         */
        DoSubmitAccumulativeRunnable(Display display) {
        	super(display);
        }
        ...
        protected void submit() {
        	final Runnable timer = new Runnable() {
				public void run() {
					DoSubmitAccumulativeRunnable.this.run();
					//run only once
					DoSubmitAccumulativeRunnable.this.display.timerExec(-1,this);
				}
        	};
            this.display.asyncExec(new Runnable() {
				public void run() {
					DoSubmitAccumulativeRunnable.this.display.timerExec(DELAY, timer);					
				}
            });
